 Пр+ограмма на Python для создания блокчейна
# Для временной метки
import datetime
...
def display_chain():
    conn = psycopg2.connect(dbname = 'duz', user='postgres', password='183305', host='localhost', port = '5432')
    cursor = conn.cursor()
    cursor.execute('SELECT * FROM "tri"')
    df = cursor.fetchall()
...
if name == "main":
    app.run(host='10.180.96.254', port=5000)